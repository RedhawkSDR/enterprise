...
import redhawk.driver.xml.model.sca.spd.Softpkg;
 ...
 public class RedhawkDomainManagerImpl extends QueryableResourceImpl<DomainManager> implements RedhawkDomainManager {
	...
	public RedhawkDeviceManager createDeviceManager(String deviceManagerName, String fileSystemRoot, boolean durable) throws Exception {
		return createDeviceManager(deviceManagerName, fileSystemRoot, durable, null);
	}
	
	@Override
	public RedhawkDeviceManager createDeviceManager(String deviceManagerName, String fileSystemRoot, boolean durable, DeviceManagerInturruptedCallback callback) throws Exception {
		final DeviceManagerTemplate devMgr = new DeviceManagerTemplate(domainName, getCorbaObj(), getOrb(), deviceManagerName, fileSystemRoot);
		
		RedhawkDeviceManager deviceManager;
		if(durable){
			deviceManager = new DurableRedhawkDeviceManagerImpl(domainName, deviceManagerName, fileSystemRoot, this, getOrb().object_to_string(devMgr.getCorbaDeviceManager()), devMgr.getCorbaDeviceManager().identifier(), callback);
		} else {
			deviceManager = new RedhawkDeviceManagerImpl(this, getOrb().object_to_string(devMgr.getCorbaDeviceManager()), devMgr.getCorbaDeviceManager().identifier());
		}
		
		driverRegisteredDeviceManagers.put(domainName+":"+deviceManagerName, deviceManager);
    ...
    public Map<String, RedhawkApplication> applications(){
    	...
    	for(RedhawkApplication app : getApplications()){
	        try {
    			...
    			/*
    			 * If something no longer exists ( for example a narrowband waveform 
    			 * that existed when getCorbaObject().applications() was called and dropped 
    			 * before application.name() was called ) then we still need the rest of the 
    			 * items which could include what we want.
    			 */
    ...
    public List<RedhawkApplication> getApplications(){
        ...
        for(Application application : getCorbaObject().applications()){
	        try {
	        	applications.add(new RedhawkApplicationImpl(this,getOrb().object_to_string(application), application.identifier()));
			} catch ( OBJECT_NOT_EXIST cf ) {
				logger.log(Level.FINE, "Item no longer exists", cf );
			}
        }
    ...
    public RedhawkApplication getApplicationByName(String name) throws MultipleResourceException, ResourceNotFoundException {
    	List<RedhawkApplication> applications = getApplicationsByName(name);    	
    	
        if(applications.size() > 1){
        	throw new MultipleResourceException("More than one application matches the search criteria.  Use getApplicationsByName instead.");
    ...
    public RedhawkApplication getApplicationByIdentifier(String identifier) throws ResourceNotFoundException{
    	List<RedhawkApplication> applications = getApplications().stream().filter(a -> {
    		return a.getIdentifier(
    				).toLowerCase().matches(identifier.toLowerCase());
    	}).collect(Collectors.toList());
    ...
    public List<RedhawkDeviceManager> getDeviceManagers() {
        ...
        for(DeviceManager deviceManager : getCorbaObject().deviceManagers() ){
        	try {
        		deviceManagers.add(new RedhawkDeviceManagerImpl(this,getOrb().object_to_string(deviceManager), deviceManager.identifier()));
	...
	public List<RedhawkDeviceManager> getDeviceManagersByName(String name) {
        ...
        for(DeviceManager deviceManager : getCorbaObject().deviceManagers() ){
        	try {
        		...
        		// if a single device manager fails, don't give up on getting the rest
        	}
        }
    ...
    public RedhawkDeviceManager getDeviceManagerByName(String name) throws MultipleResourceException, ResourceNotFoundException {
        List<RedhawkDeviceManager> deviceManagers = getDeviceManagersByName(name);
        if(deviceManagers.size() > 1){
        	throw new MultipleResourceException("More than one device manager matches the search criteria.  Use getDeviceManagersByName instead.");
        ...
        for(DeviceManager deviceManager : getCorbaObject().deviceManagers() ){
        	try {
        		if(deviceManager.identifier().toLowerCase().matches(identifier.toLowerCase()))
        			return new RedhawkDeviceManagerImpl(this,getOrb().object_to_string(deviceManager),deviceManager.identifier());
	...
	public RedhawkDevice getDeviceByName(String deviceName) throws MultipleResourceException {
        ...
        if(devices.size() > 1){
        	throw new MultipleResourceException("More than one device matches the search criteria.  Use getDevicesByName instead.");
  ...
  public RedhawkApplication createApplication(String instanceName, File sadFile) throws ApplicationCreationException {
  	if(!sadFile.exists()){
  		throw new ApplicationCreationException("The Sad file referenced by path: " + sadFile.getAbsolutePath() + " does not exist");
  	} else if(sadFile.isDirectory()){
  		throw new ApplicationCreationException("The Sad file referenced by path: " + sadFile.getAbsolutePath() + " is a directory not a file");
  	}
  	
	try {
		Softwareassembly softwareAssembly = RedhawkUtils.unMarshalSadFile(new FileInputStream(sadFile));
		String sadFileDestination = File.separator + "waveforms" + File.separator + softwareAssembly.getName() + File.separator +softwareAssembly.getName()+".sad.xml";
		//TODO: Should you be able to create a Application that's file is already in SDR???
  ...
  public RedhawkApplication createApplication(String instanceName, Softwareassembly softwareAssembly) throws ApplicationCreationException {
	try {
		String sadFileDestination = File.separator + "waveforms" + File.separator + softwareAssembly.getName() + File.separator +softwareAssembly.getName()+".sad.xml";
  ...
  public RedhawkApplication createApplication(String instanceName, String sadFileDestination) throws ApplicationCreationException {
	DataType[] initConfig = new DataType[]{};
	DeviceAssignmentType[] deviceAssignmentTypes = new DeviceAssignmentType[]{};
	
	Application applicationInstance;
	try {
		applicationInstance = getCorbaObj().createApplication(sadFileDestination,instanceName, initConfig, deviceAssignmentTypes);
		RedhawkApplication appToReturn = new RedhawkApplicationImpl(this, getOrb().object_to_string(applicationInstance), applicationInstance.identifier());
		return appToReturn;
	} catch (CreateApplicationError | CreateApplicationRequestError | CreateApplicationInsufficientCapacityError | InvalidInitConfiguration | InvalidProfile | InvalidFileName e) {
		throw new ApplicationCreationException(e);
	}	
  }
	...
	public List<RedhawkService> getServices() {
		return getDeviceManagers().stream().map(d -> {
			return Arrays.stream(d.getCorbaObject().registeredServices()).map(s -> new RedhawkServiceImpl(d, s)).collect(Collectors.toList());
	...
	public RedhawkService getServiceByName(String serviceName) throws MultipleResourceException {
        ...
        if(services.size() > 1){
        	throw new MultipleResourceException("More than one service matches the search criteria.  Use getServicesByName instead.");
    ...
    public RedhawkEventChannelManager getEventChannelManager() {
    	return new RedhawkEventChannelManagerImpl(getOrb(), getCorbaObj().eventChannelMgr());
    }

	@Override
	public RedhawkApplicationFactoryImpl getApplicationFactoryByIdentifier(String identifier) throws ResourceNotFoundException, MultipleResourceException {
		List<ApplicationFactory> factories = Arrays.stream(getCorbaObj().applicationFactories()).filter(af -> { return af.identifier().equalsIgnoreCase(identifier);}).collect(Collectors.toList());
		if(factories.size() > 1) {
			throw new MultipleResourceException("Multiple Application Factories were found with the identifier: " + identifier);
		} else if(factories.size() == 1) {
			return new RedhawkApplicationFactoryImpl(this, getOrb().object_to_string(factories.get(0)), identifier);
		} else {
			throw new ResourceNotFoundException("An application factory with the identifier of: " + identifier + " was not found");
		}
	}
	...
	public Domainmanagerconfiguration getDomainManagerConfiguration() {
		try {
			...
			return ScaXmlProcessor.unmarshal(new ByteArrayInputStream(dmdFileInBytes), Domainmanagerconfiguration.class); 
