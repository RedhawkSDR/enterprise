...
@ServerEndpoint("/{nameServer}/domains/{domainName}/eventchannels/{eventChannel}")
public class EventChannelEndpoint {

    private static Logger logger = Logger.getLogger(EventChannelEndpoint.class.getName());

	@OnOpen
	public void onOpen(Session session, EndpointConfig c, @PathParam("nameServer") String nameServer, @PathParam("domainName") String domainName, @PathParam("eventChannel") String eventChannelName) {
        ...
        if (redhawk == null) {
	        ...
	        RedhawkPort port;
			try {
				RedhawkEventChannel eventChannel = redhawk.getDomain(domainName).getEventChannelManager().getEventChannel(eventChannelName);
