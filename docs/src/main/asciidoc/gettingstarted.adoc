...
. Update the ${karaf.base}/etc/system.properties file to have the system properties necessary for JacORB.
+
----
...
. Restart karaf if it's already started. 

==== Configure JacORB logging w/ Karaf


To make sure the JacORB logging doesn't fill the general karaf.log file. Add the snippet below to the bottom of your `org.ops4j.pax.logging.cfg` which is located in ${karaf.base}/etc directory:
...
To specify specific properties add a jacorb.properties file to ${karaf.base} . For example, if you wanted to control the number of ClientMessageReceptor threads created by JacORB and manage the Idle ones you'd add a jacorb.properties file like this:
...
For additional JacORB information refer to JacORBs http://www.jacorb.org/documentation.html[Official Documentation]. 